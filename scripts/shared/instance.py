...
from my_name_utils import canonical_target_name, canonical_elemin_name, canonical_property_name
...
class Span_Instance:
    def __init__(self, docid, doc_start_char, doc_end_char, text, ner_label, sent_toks = None, sentid = None, sent_start_idx = None, sent_end_idx = None):
        """ 
        ...
        if self.ner_label == 'Target':
            self.std_text = canonical_target_name(self.text) 
        elif self.ner_label in ['Component', 'Element', 'Mineral']:
            self.std_text = canonical_elemin_name(self.text)
        elif self.ner_label == 'Property':
            self.std_text = canonical_property_name(self.text)
        ...
        self.decoded_input = [] # this is the input to bert 

        self.relation_label = None


    def insert_type_markers(self, tokenizer, use_std_text = True, max_len = 512, mask_ner = False):
        """
        ...
        if use_std_text:
            ner_spans = tokenizer.tokenize(self.std_text)
        else:
            ner_spans = tokenizer.tokenize(" ".join(self.sent_toks[self.sent_start_idx:self.sent_end_idx]))
        if not mask_ner:
            spans = ner_spans
        else:
            spans = [f"<{self.ner_label.lower()}>"]

        end_markers = [f"<ner_end={self.ner_label.lower()}>"]

        posspans = tokenizer.tokenize(' '.join(self.sent_toks[self.sent_end_idx:] + ["[SEP]"]))
        self.decoded = prespans + start_markers + ner_spans + end_markers + posspans 
        self.decoded_input = prespans + start_markers + spans + end_markers + posspans
    ...
    def __str__(self): 
        return f"DOCID: {self.doc_id}\nTEXT: {self.text}, STDTEXT: {self.std_text}, NER:{self.ner_label}, Involved in Relation:{self.relation_label}, ({self.doc_start_char}, {self.doc_end_char}), SENTID: {self.sentid}\nSENTENCE:\n    {'' if self.sent_toks is None else ' '.join(self.sent_toks) }\nDECODED: \n    {' '.join(self.decoded)}\nDECODED INPUT TO BERT: \n    {' '.join(self.decoded_input)}\nSTART, END OFFSET: ({self.doc_start_char, self.doc_end_char})\n"
